# vendor_robot_calibration 本包功能目前暂未实现。
机器人启动时会根据urdf中的模型配置参数加载并发布对应的模型，但是机器人真实的DH参数与理论设计参数存在误差----这也是为什么要进行出厂标定的原因。
本包期望实现使用机器人真实的运动学参数修正urdf中的相关参数，提高机器人运动学上的精度。
